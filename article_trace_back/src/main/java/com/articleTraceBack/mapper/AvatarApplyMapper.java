...
public interface AvatarApplyMapper extends BaseMapper<AvatarApply> {

    /** 审核分页（联查申请人信息）；status 为 null 时查全部 */
    @Select("<script>"
            + "select a.*, u.username as username, u.nickname as nickname "
            + "from avatar_apply a left join user u on a.user_id = u.id "
            + "<where><if test='status != null'>and a.status = #{status}</if></where> "
            + "order by a.create_time desc limit #{offset}, #{pageSize}"
            + "</script>")
    List<AvatarApply> selectPageWithUser(@Param("status") Integer status,
                                         @Param("offset") int offset,
                                         @Param("pageSize") int pageSize);

    /** 记录总数；status 为 null 时查全部 */
    @Select("<script>"
            + "select count(*) from avatar_apply "
