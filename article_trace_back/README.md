...
- **新增/更新**：标题 + 内容先做敏感词校验 → 内容以 JSON 形式上传 RustFS 内容桶（文件名 `时间戳-用户ID-随机后缀.json`，后缀用来防同毫秒两次保存撞名）→ 数据库仅存文件名，读取时实时从 RustFS 拉取。
- **并发编辑走条件更新**：`UPDATE ... WHERE id=? AND content=<读到的旧文件名>`——`content` 每次保存都换新对象名，正好当版本号。两人同时编辑时后写者影响 0 行，回收自己刚上传的对象并回「这篇文章刚被改过，请刷新页面后重新提交」，不会静默覆盖先写者的编辑（也不会留下孤儿正文对象）。
...
- **匹配前先归一化**（`Utils/TextNormalizer`）：去掉空白与零宽字符 → 全角 ASCII 压半角 → 统一小写。AC 是**逐字符精确匹配**，`赌 博` / `赌　博` / `赌&#8203;博` 这类插了干扰字符的写在归一化之前一条都命中不了——这是词表最容易绕过的一环。归一化只作用于**送进匹配的那份副本**，落库原文与返回给作者的文本都不变（命中记录的是词，与位置无关）。
...
| `profile.impersonation-words` | `站长,管理员,管理員,官方,客服,认证` | 昵称 / 个签里禁止出现的冒充词，逗号分隔；站名另由 `site.display-name` 兜底。线上可用 `PROFILE_IMPERSONATION_WORDS` 覆盖，通常不必动 |
