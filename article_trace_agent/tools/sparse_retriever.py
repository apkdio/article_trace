...
def _matches_filter(doc: Document, f: dict) -> bool:
    ...
    支持 {"category_id": {"$eq": 3}}、{"$and": [...]}、$gte/$lte/$eq 比较。
    """
    if not f:
        return True
    if "$and" in f:
...
class SparseRetriever:
    ...
    def search(
        ...
        for rank, (doc, score) in enumerate(results, 1):
            ...
            logger.debug("  [Sparse #%d score=%.4f] %s | %s", rank, score, title, preview)
        return results
    ...
    def _fingerprint() -> dict:
        ...
        return {
            ...
            "collection_name": _chroma_cfg.get("collection_name", "article_kb"),
        }
