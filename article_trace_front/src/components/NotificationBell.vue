 ...
 * 站内通知铃铛 + 抽屉。
 *
 * 站内信在两处布局里都要出现（后台布局与门户页），所以把「未读数轮询 + 抽屉 + 已读/删除」
 * 收在这一个组件里 —— 复制两份的话，轮询逻辑一旦不一致就会出现「一边响一边不响」。
 *
 * 铃铛外观由父组件决定（作用域插槽暴露 open / unread）：
 * - 不传插槽时用默认的图标样式（后台布局沿用）
 * - 传插槽可换成圆形按钮等，与所在页面的按钮风格保持一致
 */
